...
public enum FetchActionId
{
    ...
    /// <summary>
    /// 按沪深交易所官方份额「成交量 ÷ 前一交易日份额」重算两市 ETF 换手率（2026-09-23，
...
public static class FetchTaskCatalog
{
    ...
    [
        ...
        new(FetchActionId.StepEtfTurnoverFix, "ETF换手率校正", "沪深交易所官网（ETF 份额）+ 本地计算", QuotaGroup.Exchange,
            ...
            + "同一天两个接口还可能不一样（510150 在 2024-09-30：前复权 83.63、不复权 48.38），"
            + "这种被全库体检报成「多口径不一致」，【重新拉取失败】修不好——同一时刻重抓还是那个数。"
            + "深市腾讯一直按前一交易日算。两市都有 2022 年几个故障日（09-09/13/14、10-11/12）用的份额比实际更旧。"
            + "② 腾讯的 ETF 换手率大约 **2022 年年中**才开始有，之前一律是 0：沪市约 24 万行、深市约 11 万行有成交却没有换手率，"
            + "两个口径都是 0，体检看不出来。\n"
            + "**凭什么能算**：两市全部 ETF 逐行复算过——腾讯给的非 0 值，要么等于「÷前一交易日份额」、要么等于「÷当天份额」"
            + "（例外只有上面那几个故障日），都能用官方份额精确复算到 0.01。"
            ...
            + "首轮写回 143 万行也只要 44 秒。\n"
            + "⚠ 要排在【ETF日K】【ETF日K·不复权】之后：它检查的就是当天新抓的这两套。"
            + "跟【重算回测序列】没有先后要求——回测序列的换手率原样抄不复权日K，谁先谁后结果都对。",
            FetchActionParams.None,
    ...
    [
        ...
        // 【ETF换手率校正】（2026-09-23 从按需挪进日更）。**唯一的前置是 ETF日K / ETF日K·不复权**
        // （在前面的K线族里）：它检查的就是当天新抓的这两套。
        // ⚠ 跟【重算回测序列】**没有先后依赖**：那一项写 day_adj 时换手率原样抄 day_raw
        //   （增量和整段重算都是），校正先跑＝抄过去的就是对的值，后跑＝新 day_adj 行顺带改掉。
        //   排在这一批末尾只是因为它不是"当天必须拿到"的数据，别挡前面那些。
