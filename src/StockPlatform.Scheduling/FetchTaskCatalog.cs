...
public enum FetchActionId
{
    ...
    /// <summary>
    /// 按上交所官方份额「成交量 ÷ 前一交易日份额」重算沪市 ETF 换手率（2026-09-23，
    /// 见 doc/etf-turnover-recalc-design.md）。增量只报告、彻底重查才写回。
    /// </summary>
    StepEtfTurnoverFix,
}
...
public static class FetchTaskCatalog
{
    ...
    [
        ...
        new(FetchActionId.StepEtfTurnoverFix, "ETF换手率校正", "上交所官网（ETF 份额）+ 本地计算", QuotaGroup.Exchange,
            TimeSpan.FromMinutes(5), "按需",
            "用上交所官方公布的每日 ETF 份额，按「**成交量 ÷ 前一交易日份额**」重算**沪市 ETF** 的换手率，"
            + "把错值改对、空值补上。只改换手率一列，价格和量额一行不动。\n"
            + "**修的是什么**：① 腾讯的 ETF 换手率**口径不统一**——2024-10 以前几乎全按前一交易日份额算，"
            + "2024-11 起多数改按当天份额，但仍夹着成片按前一交易日算的（2025-05、2026-07/08），"
            + "2022 年还有 4 天用的份额比实际更旧；同一天两个接口还可能不一样（510150 在 2024-09-30：前复权 83.63、不复权 48.38）。"
            + "后者被全库体检报成「多口径不一致」，【重新拉取失败】修不好——同一时刻重抓还是那个数。"
            + "② 腾讯的 ETF 换手率大约 **2022 年年中**才开始有，之前一律是 0：沪市约 29.8 万行有成交量却没有换手率，"
            + "两个口径都是 0，体检看不出来。\n"
            + "**凭什么能算**：2012 年起全部沪市 ETF 逐行复算过——腾讯给的每一个非 0 值，要么等于「÷前一交易日份额」、"
            + "要么等于「÷当天份额」（例外只有上面那 4 天的 539 行和零星二十来行），两种都能用官方份额精确复算到 0.01。"
            + "统一成前一交易日（2026-09-23 用户定）：收盘后马上就能算（当天份额常常第二天才发布），回测也不会用到收盘后才公布的数。"
            + "东财（拿**当前**份额除历史成交量）和搜狐的换手率都对不上任何一天的官方份额，不能当裁判。\n"
            + "**两档**：「日常增量」＝补份额 + 只检查、**不写库**，日志里报错值/空值/无法裁判各多少行并列样例；"
            + "「彻底重查」＝同上再写回。先跑增量看报告，数字对了再切彻底重查。\n"
            + "**不做什么**：上交所 2012-01-04 起才有份额数据，更早的行、上市首日（没有前一天份额）、"
            + "上交所那天没列的，都原样留着、单独报数。**深市暂不处理**——深交所的份额源还没核。\n"
            + "**请求量**：首次要补 2012 年起约 3,600 个交易日的份额，一天一个请求、一天一批落库，约 1 小时，"
            + "停在哪都不丢；之后每次只补新的交易日。份额存在 EtfShare 表，也能拿来看 ETF 规模和每日申赎。\n"
            + "⚠ 腾讯眼下多数按当天份额给，所以**新抓进来的 ETF 日K会继续跟这个口径有出入**，而且两个口径通常一起偏、"
            + "体检报不出来。隔一阵跑一次「日常增量」看报告里的错值数，攒多了再跑一次「彻底重查」。",
            FetchActionParams.None,
            // 软前置：份额要按交易日历逐日拉；检查的是 ETF 两套日K。缺了照样能跑（能查多少查多少），
            // 只是新的那几天查不到。
            SoftDependsOn: [FetchActionId.StepTradingCalendar, FetchActionId.StepEtfBars, FetchActionId.StepEtfRawBars],
        ...
        new(FetchActionId.RepairQfq, "重取前复权", "腾讯", QuotaGroup.Mixed,
            ...
            + "2026-09-22 迁到新任务框架（QfqRepairTask）：一批 10 只、**每批存完就从名单划掉**，"
            + "所以中途停（空闲窗口到点、限流打断、按停止）不再白跑——"
            + "以前划账在整批跑完之后、取消直接冒泡，已经重写完的票下一轮会全部重抓一遍。",
