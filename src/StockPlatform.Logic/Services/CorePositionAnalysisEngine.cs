...
/// 按持有 1/3/5 年分别统计——这是独立的一个活，**还没做**。做的时候二选一，别混：
///   · 用 <see cref="Granularity.DayAdj"/>（回测序列）**直接**算总收益——它已经把分红按再投计进去了，
///     **不能再加 Dividend 表的实收分红，会重复计算**；
///   · 或者用 <see cref="Granularity.Day"/> 算纯价差，另外加 Dividend 表的实收分红——
///     这条更贴近"钱真的进了账户"的口径，底仓法本来就是冲着现金流去的。
/// 别用 day_hfq：它是混合式的，把高股息股的收益率系统性压低（实测 5% 分位 ×0.754），
/// 而底仓池全是高股息股，正好踩在最不准的那一档上。
///
/// ════ 硬条件（AND，全部满足才入选）════
///   ① 净利润为正（最新一期财报）
///   ② 经营现金流为正
