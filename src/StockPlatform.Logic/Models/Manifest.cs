...
public class Manifest
{
    ...
    /// <summary>
    /// 【板块指数合成】下一轮要**整段重算**（2026-09-21）。
    ///
    /// 板块指数是拿成分股的 <c>day_adj</c> 累乘出来的，所以【重算回测序列】一旦重写了因子，
    /// 板块指数的**整条历史**都失效了——而合成那一项自己看不出来（它只比得出名单变没变、
    /// 成分股有没有补更早的历史）。所以由重算那一项跑完置这个标记，合成看到就整轮全量、然后清掉。
    ///
    /// ⚠ 漏了这条通路的后果是**静默的错值**：板块指数只有本地这一份，没有官方值可对。
    /// </summary>
    public bool BoardIndexNeedsFullRebuild { get; set; }
