...
public class WatchlistEntry
{
    ...
    /// <summary>我自己给这只票打的分类（2026-09-22新增，"主动仓"页手填）——比如"Apple供应商"、
    /// "光模块"、"红利"。纯自由文本，程序不解释它的含义，也不参与任何选股/纪律判断：本地已有的
    /// 行业（申万）和概念板块（东财）都是别人的分法，这一列是**我自己的分法**，两者互不替代。
    ///
    /// 一只票想归多个类就用顿号分隔（"Apple供应商、光模块"），跟"板块"列的写法一致——没有做成
    /// 多值结构，是因为当前只需要"看得见、能按它排序"，结构化留到真要按分类筛选/汇总时再说。
    /// 空字符串 = 没分类（老数据里没有这个字段，反序列化即为空）。</summary>
