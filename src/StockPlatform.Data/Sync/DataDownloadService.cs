...
namespace StockPlatform.Data.Sync;

/// <summary>
/// "从服务端更新数据"的跨平台版本——跟分析程序里的 DataSyncService 同一套逻辑，但只依赖两个显式路径
/// （目标库 + 工作目录），不绑定 Windows 的 AnalyzerPaths，好让手机端(Android/Avalonia)直接复用。
///
/// 逻辑：本地没有库→下载最新全量基线(baseline-*.zip)解压成目标库；之后→只下比本地新的每日增量
/// (daily-*.zip)按日期合并进去。本地"数据到哪天"直接查库里最新日线日期，不另存状态。
/// </summary>
public class DataDownloadService
{
    ...
    private readonly string _totalDbPath;
    private readonly string _workDir;

    public DataDownloadService(string totalDbPath, string workDir)
    {
        _totalDbPath = totalDbPath;
        _workDir = workDir;
    ...
    public async Task UpdateAsync(IProgress<string> status, CancellationToken ct = default)
    {
        ...
        status.Report("连接服务端，读取可用数据…");
        var assets = await client.ListAssetsAsync(ct);
        if (assets.Count == 0) { status.Report("服务端还没有任何数据（需要发布端先上传全量基线）。"); return; }
        ...
        if (needBaseline)
        {
            if (latestBaseline == null) { status.Report("本地没有数据、服务端也没有全量基线，无法初始化。"); return; }
    ...
    {
        var zipPath = Path.Combine(_workDir, "_baseline_download.zip");
        ...
        ExtractFirstSqlite(zipPath, _totalDbPath);
        File.Delete(zipPath);
    }
    ...
    {
        var zipPath = Path.Combine(_workDir, "_daily_download.zip");
        var tmpDb = Path.Combine(_workDir, "_daily_download.sqlite");
        ...
        SqliteConnection.ClearAllPools();
        SqliteMerger.MergeInto(_totalDbPath, tmpDb);
        File.Delete(zipPath);
        File.Delete(tmpDb);
    }
    ...
    {
        if (!File.Exists(_totalDbPath)) return null;
        try { return new SqliteBarRepository(_totalDbPath).GetOverallLatestPeriodStart(Granularity.Day); }
        catch { return null; }
    }
