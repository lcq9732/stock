...
/// **只读** current.sqlite。
///
/// ════ 跟 <see cref="SqliteWatchReadingSource"/> 的分工 ════
/// 那个是给**求值**用的：取一个数、判要不要触发、要不要提醒。
/// 这个是给**阅读**用的：把来龙去脉讲成人话，尤其是回购那种跨几个月的过程。
/// 两者取的是同一批表，但形状完全不同，合成一个会互相将就。
///
/// ════ 谁进个股行、谁进市场观察 ════
...
public class SqliteStockEventSource
{
    ...
    /// L0 当初为同一个原因加过时间窗（见 WatchRuleEngine），叙述这条线也得有。
    ///
