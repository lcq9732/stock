...
public static class SqliteSchema
{
    public static void EnsureSchema(SqliteConnection conn)
    {
        ...
        cmd.CommandText = """
            ...
            -- 分红送配的**逐只抓取状态**（2026-09-18）——【拉取分红送配】的水位线。
            --
            -- 为什么 Dividend 表本身当不了水位线：没分红的票一行都不写，「没抓过」和
            -- 「抓过、确实没分红」长得一模一样（全市场约三千只是后者）。按行数判会让它们每轮全抓，
            -- 而这一项一轮就是 5800 个请求、半小时起，中途被限流打断就等于整轮白跑
            -- ——2026-09-18 凌晨那次正是如此，停止之后连失败名单都没留下。
            --
            -- 粒度是**只**，细于骨架的截断粒度（一批 30 只），满足 CustomerSupplierTask 立的那条
            -- 一般原则：任务的水位线必须细于骨架的截断粒度，否则被 Deadline 截断的那一批下轮会被
            -- 当成「抓过了」跳过，而且毫无征兆。
            --
            -- last_ok_at 只记**成功**那次：失败不算抓过，下一轮还得来。
            -- dividend_rows=0 是「那次确实没有」的事实记录，不是「以后别再抓了」——
            -- 新股第一次分红就会变，所以这张表只管陈旧度，不做永久跳过。
            CREATE TABLE IF NOT EXISTS DividendFetchState (
                code          TEXT PRIMARY KEY,   -- 6位股票代码（含退市股）
                last_ok_at    TEXT,               -- 上次抓成功的时刻；失败不更新
                dividend_rows INTEGER,            -- 那次拿到几条分红（0 = 确实没有）
                rights_rows   INTEGER,            -- 那次拿到几条配股
                last_fail_at  TEXT,               -- 上次失败时刻（诊断用）
                fail_reason   TEXT                -- 上次失败原因（诊断用）
            );
            ...
            --     全量 264 万行就是丢十几万。
            --
            -- ⚠ seq 是**位次**、不是稳定标识（2026-09-17）：它按净额降序在落库时自赋，取值取决于
            --    "这一组里有几行"。所以**幂等不能靠主键 UPSERT**——只要某次抓取多收了一行，整组编号
            --    就多一位，上次落库的高位 seq 行没人覆盖得掉，永久留在库里。全表曾因此堆出 3579 行
            --    副本（同时缺着一样多的真行，都是按月切片深分页 + 非唯一排序键的跨页错位造成的）。
            --    现在落库走 ILhbSeatRepository.ReplaceForDay（**整日替换**），原来的 Upsert 已删除。
