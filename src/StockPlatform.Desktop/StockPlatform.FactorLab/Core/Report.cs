...
public static class Report
{
        ...
        double[,] corr, Dictionary<int, string?> dedup, IReadOnlyList<PortfolioResult> portfolios, List<string>? picks)
    {
        ...
        sb.AppendLine();
        string head = $"- 数据：{md.Dates[0]} ~ {md.Dates[^1]}（{md.NDays} 个交易日，{md.NStocks} 只 A 股";
        sb.AppendLine(md.PriceGranularity switch
        {
            "day_adj" => head + "，**回测序列 day_adj**：不复权原始价 × 本地算的乘法式复权因子，"
                              + "非除权日的收益率精确等于真实收益率）",
            "day_hfq" => head + "，**数据源后复权 day_hfq**）\n"
                       + "- ⚠️ **警告：库里没有 day_adj，本次降级用了数据源的后复权。** 它是"
                       + "「送转乘、分红加」的混合式，加法项会阻尼波动，把**高股息股**的收益率系统性压低——"
                       + "实测非除权日收益率÷真实 中位 0.967、5% 分位 0.754；十年累计更明显："
                       + "农业银行 157% vs 真实 293%、工商银行 112% vs 214%。"
                       + "**红利/高股息类因子的结论会被系统性低估**，请先在 Fetcher 跑一次"
                       + "【补不复权历史】+【重算回测序列】。",
            _ => head + "）\n"
               + "- ⚠️ **警告：库里既没有 day_adj 也没有 day_hfq，本次降级用了前复权。** 数据源的前复权是"
               + "减法式，回看越久高分红股的复权价越接近零甚至为负，收益率会严重失真（实测2016~2019有27%的"
               + "股票出现过物理上不可能的单日涨跌）。**本报告的长周期结论不可信**，"
               + "请先在 Fetcher 跑一次\"拉取区间数据\"。",
        });
        ...
        // ⚠ 这段警告对 day_adj 和 day_hfq **都成立**：两者都是乘法式复权，都把现金分红计进了因子，
        //   所以拿复权价去折算历史市值都会偏低。换成 day_adj 只是让偏差的量级变了，没有消除它。
        //   真要修干净，得用 day_raw（真实成交价）折算、再单独乘一个**纯送转**因子来还原历史股本——
        //   day_raw 是 2026-09-01 才有的，这个改进还没做。
        sb.AppendLine("- ⚠️ 历史市值是**近似值**：用「最新流通市值 ÷ 最新收盘 × 当日收盘」折算。"
                      + "**送转不是问题**（复权因子与股本变动是同一个乘数，已被正确抵消）；残留误差有两处："
                      + "①现金分红计入复权因子，使历史市值被系统性低估（高股息股最明显；day_adj 与 day_hfq "
                      + "都有这个问题，只是量级不同）——要修干净得改用 day_raw 折算再单独还原股本，还没做；"
