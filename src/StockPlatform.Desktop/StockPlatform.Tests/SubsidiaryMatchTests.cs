...
{
    private static (Dictionary<string, string> ByFull, Dictionary<string, string> ByNorm) Listed()
    ...
    public void 子公司名归并到母公司()
    {
        ...
        var (code, type) = PartnerNameMatcher.Match("中国建筑第六工程局有限公司", f, n, subs);
    ...
    public void 上市公司本尊不许被归并走()
    {
        ...
        // 前两档先命中，轮不到第三档——它是它自己
        var (code, type) = PartnerNameMatcher.Match("比亚迪股份有限公司", f, n, subs);
    ...
    public void 一个名字落到两个母公司_整个丢掉()
    {
        ...
        var (code, type) = PartnerNameMatcher.Match("某某科技有限公司", f, n, subs);
    ...
    public void 归一化后也能命中()
    {
        ...
        var (code, type) = PartnerNameMatcher.Match("中国建筑第六工程局", f, n, subs);
        Assert.Equal("601668", code);
        Assert.Equal(PartnerNameMatcher.Subsidiary, type);
    }
    ...
    public void 匿名占位符不参与第三档()
    {
        ...
        {
            var (code, _) = PartnerNameMatcher.Match(anon, f, n, subs);
            Assert.Null(code);
        }
    }
    ...
    public void 不传子公司索引时行为不变()
    {
        var (f, n) = Listed();
        // 老调用点一个参数都没改，必须还是老行为
        var (code, type) = PartnerNameMatcher.Match("中国建筑第六工程局有限公司", f, n);
        Assert.Null(code);
        Assert.Null(type);
    }
}
