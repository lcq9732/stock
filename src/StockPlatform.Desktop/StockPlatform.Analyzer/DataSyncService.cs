...
public class DataSyncService
{
    ...
    public async Task UpdateAsync(IProgress<string> status, CancellationToken ct = default)
    {
        ...
        status.Report("正在从服务器读取数据…");
        var assets = await client.ListAssetsAsync(ct);
        if (assets.Count == 0) { status.Report("服务器上还没有任何数据（需要发布端先用 Fetcher 上传全量基线）。"); return; }
        ...
        if (needBaseline)
        {
            if (latestBaseline == null) { status.Report("本地没有数据、服务器上也没有全量基线，无法初始化。"); return; }
