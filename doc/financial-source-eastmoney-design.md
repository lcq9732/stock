...
## 4c. 决策：换（2026-09-10 拍板）

193 只票、508,681 格逐格比对之后切换。**决定性的不是格数，是"该信谁"这个先验**：

> 东方财富有**持牌券商**（东财证券）、天天基金和行情终端，客户拿它的 F10 做买卖决策，
> 数据错了有人投诉、有合规压力。新浪财经是资讯门户，**没有交易业务**，F10 错十年也没人报。

这条先验还**预测了实测到的数据形态**，两边正好对上——东财是"近强远弱"：

| 年代 | 东财缺失率 | 用"有客户在用"解释 |
|---|---|---|
| 2020 年后 | 0.2% | 客户天天看，必须准 |
| 2010-2019 | 0.8% | 还有人翻 |
| 2005 年前 | 4~6% | 没人看，就薄 |

而今天查清的**每一个个案，错的都是新浪**，没有例外：117,860 格幽灵 0、数字截到万位、
银行营业支出漏掉信用减值（462 亿 vs 东财 1166 亿，且东财的营业收入与年报一致）、
以及同日发现的行业分类整组错位 557 只。两条独立的证据链指向同一个结论。

**接受的代价**：老年代数据变浅（2005 年前缺 4~6%，外推全市场约 16 万格）。
理由是那时上市公司才几百家、会计准则也完全不同，对回测价值有限；
换来的是清掉约 350 万格幽灵 0、修掉银行营业支出这类系统性错误、精度从万位提到元。

**没有逐条查完 16,999 格"严重差异"就动手**，这是有意识的取舍：按上面的先验，
那批差异里相当大一部分本来就是新浪的错，换源等于顺带修数据；真要逐格核对年报，
成本远超收益。留下 `FinancialParityTests` 作为可重复的回归工具，日后发现问题再查。

### 落地动作

1. `FinancialSource` 默认 → `eastmoney`
2. `FinancialKeys.Version` 4 → **5**：科目集没变，纯粹为了**逼全库重抓**——
   新旧两源精度和口径不同，同一张表里前后期混着两个源的值，同比/环比会出现无从分辨的断层
3. `FetchTaskCatalog`：`QuotaGroup` → `Mixed`，`Sources` → `[Sina, EmDataCenter]`（保险仍走新浪）
