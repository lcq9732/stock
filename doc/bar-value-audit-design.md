...
## 16. 修复路径的三个缺陷（一、二已修；三 2026-09-12 新发现）
...
**已实现（2026-09-11）**：`OnlyInconsistent` 那条路**额外抓一次基准 `day`**，四份都来自同一时刻
才能一致。计划本身（挑出抓不了的、按 (票,口径) 分组、追加基准重抓）抽成了 Logic 层纯函数
`ValueIssueFixPlan`，`FetchOrchestrator.Steps.cs` 只剩执行；回归用例见
`ValueIssueFixPlanTests`（10 个，含"同票两口径同区间基准只抓一次""整段重抓那条路不抓基准"）。
下面是当时的方案原文：
...
**用户 2026-09-12 定：按 A**，理由是"腾讯给数据时就会差 100，这是源的刻度"。已落地在
`SqliteBarValueAuditor.AmountQuantumYuan`（= 100 元，带出处注释），用例见 `BarValueAuditTests`
的 `V3_成交额差一个刻度不算不一致` / `差两个刻度照样报` / `放宽刻度之后量额差100倍照样报`——
放过的是**绝对一格**，不是比例，所以单位错（手/股）那种相对误差 99 的照样必报。
下面是当时的取舍原文：
...
### 缺陷三：`FindGaps` 的存续区间用口径**自己**的首末，头尾整段缺发现不了

**现象**（2026-09-12）：7 只 2023-08 上市的科创板新股，`day_raw` 从**上市首日**起整段缺
（688347 缺 18 天、688671 16、688592 11、688548 12、688573 10、688693 9、688591 4，共 80 行），
既不在待补名单、也不在「确认没有」白名单里——体检对它们完全失明。

**根因**：`SqliteMissingBarRepository.FindGaps` 的 `rng` 子查询是

```sql
SELECT code, MIN(period_start) lo, MAX(period_start) hi FROM Bar
WHERE granularity = $g AND code IN (...) GROUP BY code
```

存续区间取的是**这个口径自己**的首末。头部缺一段，`MIN` 就变成"缺完之后的第一天"，判据于是
认为这只票那天才上市，**头部永远不算缺**；尾部同理。四个口径各算各的起点，只要某个口径的
整段缺在两头，这条判据就看不见。

**连锁后果**（这次就是这么暴露的）：【统一成交量单位】的 `SameDayPredicate` 要拿同日 `day_raw`
当参照，`day_raw` 那几天压根没有行 ⇒ `EXISTS` 不成立 ⇒ 这些天的 `day`/`day_hfq` 也一起漏改，
于是同一只票上出现"`day` 是股、别的口径是手"这种**方向相反**的不一致。V3 把它报成
`inconsistent`（72 行），但修复路径重抓的是被报的非基准口径，错值却在 `day` 上——又绕回缺陷一。

**已补的数据**（2026-09-12，沙箱直连腾讯 `newfqkline` 不复权端点，源数据一行不缺）：
80 行 `day_raw` 补入；随后按产品判据（`SameDayPredicate` / 量额比）把连带漏改的单位修正——
`day` 12 行、`day_hfq` 82 行、`day_adj` 2 行、`day_raw` 2 行。688/689 全号段 V3 复查**全干净**。

**判据本身还没改**：存续区间要改成跨口径取（以 `day` 的首末为准，或四口径并集），否则下次
同样的漏抓照样发现不了。
