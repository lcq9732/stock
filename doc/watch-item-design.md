...
**⚠ 第二条硬约束：两边的标题配不上，正文就取不到（2026-09-17 体检发现）**

复用东财那条正文通道的代价：两个站**不共享公告 ID**，只能拿「巨潮给的标题 + 日期」去
东财「该股最近 100 条公告」里**猜**是哪一篇。而两边对同一篇公告的标题写法并不一致：

| 巨潮 | 东财 | 差在哪 |
|---|---|---|
| 中创智领**（郑州）**工业技术集团… | 中创智领**(郑州)**工业技术集团… | 全角 vs 半角括号 |
| 上海豫园旅游商城**（集团）**… | 上海豫园旅游商城**(集团)**… | 同上 |
| 艾迪精密**␣**关于股份回购实施结果… | 艾迪精密关于股份回购实施结果… | 巨潮标题里多一个空格 |
| …暨回购**␣B␣**股股份的进展的公告 | …暨回购B股股份的进展的公告 | 同上 |

原来的配对只做「去掉『股票名：』前缀 + 互相包含」，这四种全配不上，于是走软失败分支——
**标题落库、金额/股数/比例全 null**。09-17 那轮报的「6 条没取到正文」，6 条全是这两类，
公告在东财其实都有；全库 1959 条里 38 条空壳，34 条同因。

改法：比较前先归一化——全角转半角（FF01–FF5E 整段减 0xFEE0，括号/冒号/逗号/百分号一次全覆盖）
+ 删掉所有空白 + 转小写。**不能松到把不同的公告也配上**：同日发的《…前十名股东持股情况的公告》
必须仍然配不上回购报告书，否则会抽到另一篇的数值——那比配不上更糟，因为看不出错。

**⚠ 第三条：纯 B 股要走 `ann_type=B`**

东财公告列表按市场分通道。深 200xxx／沪 900xxx 传 `ann_type=A` 返回 `total_hits:0`，
**不报错、只是这类票的正文永远取不到**（库里 200512 闽灿坤B 的 4 条空壳就是这么来的）。
判定走 `MarketClassifier`，不在 provider 里自己写代码段规则（见 `feedback_market_prefix_via_classifier`）。
A+B 两地挂牌的票（600679 上海凤凰）不受影响：代码是 A 股代码，B 股公告也在 A 列表里。

**⚠ 空壳行增量窗口再也不会路过**——水位线是 `MAX(announce_date)`，早越过那天了。
所以配对修好之后还要有**回补**：每轮跑完新公告，再挑一批 `art_code` 为空、且公告日在 90 天内的旧行
（更早的本来就不在东财那 100 条里，重试是白费请求），按 code+标题直接取一次正文补上数值。
`stage` 只从标题定，重抽的主键不变，是就地 upsert，不会多出一行。
...
⚠ **补一句（2026-09-17）**：上面这段的前提是「`art_code` 都有值」，当时那批样本确实如此，
于是"抽不出数"被整个归因到正则。后来体检发现还有**第二类**：`art_code` 为空，
即那篇公告压根没配上、正文没取到（见 6.6 第二、三条硬约束）。
看一条空记录时这两类要分开看：`art_code` 有值＝正文到手、正则没匹配；
`art_code` 为空＝根本没拿到正文。**两者都不是"这家公司没在回购"**。
...
| 取到正文 | 95%（⚠ 缺的那 5% 后来查清了：不是上限，是标题配对没归一化 + B 股走错通道，2026-09-17 已修，见 6.6） |
...
### 「档」（Priority）整套删除（2026-09-14 晚）

A=推送 / B=进日报 / C=只落库——**一个字母决定这条消息走哪个出口**。
查下来：**那三个出口一个都没做过**，全库没有任何代码消费触发记录。所以它从建成起
就是个没有下游的路由键，实际产出只有界面上那个 32px 的字母。

而观察项页改成事件叙述之后，它连那点价值也没了：

> 档是把「量」压成一个字母。「限售解禁 789 股，占流通 0.02%」这种句子已经把量写在脸上了，
> **在显示原始数字的地方做压缩，只会丢信息**。

康辰药业解禁 **74 股**（两千块钱）曾被标成 A 档、跟"占流通 30%"同一个字母，就是压错的证据——
后来加 `WatchPriority.ForShareLift` 按占比分档来补救，但在一个已经显示占比的页面里，
这层补救本身就是多余的。

删掉的东西：`WatchPriority` 及其测试、`WatchItem.Priority`、`WatchHit.Priority`、
`WatchReading.Magnitude`、`StockWatchEvents.Priority`、`WatchEventComposer.TopPriority`、
`WatchRunResult.TopHits` / `CodePriority`、`L0Events` 的档位列、界面上的「档」列和「A 档 N 条」。

**哪天做日报/推送，档该在推的那一刻现算**，不必存进观察项、更不必显示。

⚠ 连带后果：`Run()` 现在**没有任何界面下游**了——`items.json`（在盯什么）和 `hits.json`
（报过什么）都没有 UI。`BuildEvents()` 也不再需要 `Run()` 的结果，事件叙述是纯读库的。

### 提醒只给**持仓**，不给全部跟踪（2026-09-14 晚）

起因是用户问的一句话：

> 假如此股我已经建仓了，那需要每天关注重要事件。假如此股我没有建仓，那我应该在建仓前看下此股的情况。
> 所以是否要统一提醒那么多，还是说只提醒建仓的就可以？

拿真实数据一查，答案自己就出来了：

| 清单 | 数量 |
|---|---|
| 自选股（算法验证样本） | 61，其中 31 只连主动仓都不在 |
| 主动仓 | 30，但买过的只有 8 只 |
| 主动仓里**买了还没卖完** | **4**（宁德、格力、羚锐、华域） |
| 底仓 | **6** |
| **真正有钱在里面**（去重） | **9** |
...
| | 建仓前 | 建仓后 |
|---|---|---|
| 动作 | **查** —— 我主动去问这只票 | **盯** —— 它主动来找我 |
| 用什么 | 【分析详情】（任何票随时能开，不用先加自选） | 观察项页 |
| 覆盖 | **任何票** | **只有持仓** |

"查"那件事已经被【分析详情】覆盖了，所以观察项页只服务"盯"。

⚠ 自选股里没进主动仓的 31 只，**两个范围都不含**——自选股页面上自己写着
"各选股方法丢进来的【算法验证样本】，只用来统计各方法准不准，**不代表要买**"。
对验证样本发提醒，定义上就是错的。

**范围定义**（`WatchScope`）：
- `Holding`（默认）＝ 主动仓 `IsHoldingPosition` ＋ 底仓全部
- `Tracked` ＝ 主动仓全部 ＋ 底仓全部

⚠ 持仓判据**复用 `WatchlistEntry.IsHoldingPosition`**（晨检用的就是它，语义是"买过且没平完，
部分卖出仍然算持仓"）。这种判据只该有一份——另写一个 `Shares > 0 && SellDate == null`
迟早会跟晨检说法不一。底仓一律算持仓：进了底仓清单就是打算长期拿的，
哪怕还没录买入记录（实测 6 只底仓里 5 只 `Lots` 是空的）。

### 顺带修掉一个静默 bug

`WatchService.Run` 原来取的是**整份 watchlist.json**，没有过滤 `InTradePool`：

```csharp
foreach (var e in _watchlist.Load())      // ← 61 只全进来了
    active[e.Code] = e.Name ?? "";
```

于是 61 只验证样本全被当成主动仓，而规则引擎给主动仓挂的是**短线纪律（跌破 MA20）**——
等于拿短线止损口径盯着一堆本来就不打算买的票。界面上完全看不出来。

### 为什么最后没做 `first_seen` / NEW 标记

一度定了方案：7 张表加 `first_seen`（首次入库时间）＋ Analyzer 侧存"上次看到哪"的水位线，
`NEW = first_seen > 水位线`。查证过程留个记录，因为结论是反直觉的：

- `fetched_at` **不是**"第一次见到"，是"最后一次写入"（代码里 `fetched_at = excluded.fetched_at`）。
  实测：Dividend 全表 80,021 行的 `fetched_at` 都是 09-07 同一天，Lhb 99% 是 09-10。
  **跑一次全量重抓，全市场几万条事件会集体变成"今天新的"。**
- `ShareLift` 是**全量重取**（含未来计划，东财随时改），每次 31,525 行全部 `INSERT OR REPLACE`；
  `HolderChange` 增量但**回看 30 天**补滞后字段。两者都会把 `first_seen` 抹掉，必须写成
  `COALESCE((SELECT first_seen FROM … WHERE 主键匹配), $now)`。

**范围缩到 9 只之后，用户判断这套不值得做**——9 只票几十行，每天早上扫一遍就读完了，
而且叙述本来就按日期倒序，最新的在最上面。

⚠ **残留风险（已知并接受）**：公司昨天发的公告，任务今天才抓到，那行显示的是**昨天的日期**。
按日期扫的人可能以为"昨天那条看过了"而跳过。滞后最多一天，9 只票的量级下接受。
哪天要补，就按上面那套做。

### 那台机器整体退休（2026-09-15）

`Run()` 这条线——规则引擎重算派生观察项（自动挂/摘）→ 取值器逐条求值 → 判触发 →
落 `watch/items.json` 和 `watch/hits-yyyy.json`——**整套删掉**。

它是为**推送 / 日报**造的：档位 A=推送、B=进日报、C=只落库，一个字母决定走哪个出口。
**那三个出口一个都没做过**（全库搜过，没有任何代码消费触发记录）。
观察项页改成事件叙述之后，"在盯什么"和"报过什么"这两份清单也没了界面——
叙述本身已经把内容摆出来了，中间那层只剩没人读的 json。

删掉的：`WatchRuleEngine`、`WatchEvaluator`、`SqliteWatchReadingSource`、`JsonWatchItemStore`、
`WatchItem`/`WatchHit` 模型、`WatchItemDisplay`、`WatchRunResult`、`AnalyzerPaths` 的两个路径属性，
以及它们的三个测试文件。

⚠ **没删、也不能删**：`WatchIndicatorRuleEngine` + `WatchIndicatorRuleTask` +
`SqliteWatchIndicatorRepository` + `StockWatchIndicator` 表。那是 Fetcher 侧的
"哪只票该挂哪个行业指标"，碳酸锂能出现在宁德行里全靠它，`SqliteStockEventSource` 直接查那张表。
名字像，是两条完全不同的线。

⚠ 旧的 `watch/items.json` 和 `hits-yyyy.json` **不删**——它们是历史。

════ 更正一处我说过头的话 ════

退休前我说过"触发流水停了就补不回来——stage 跃迁要靠上一次观察到的状态"。**说重了。**
实际代价只是：将来真做晨间提醒时，第一次运行会把所有现状当成"新跃迁"报一遍，
之后就正常了。而那个一次性噪音用 `first_seen` 或"首次运行不报"都能规避。

而且真要做，`first_seen` 那条路比恢复这套旧机器更直：
**"这行第一次进库是什么时候"是数据的事实**，而旧机器把事实和"我看过什么"混在一起存，
在重抓 / 补历史 / 中断续抓这三件事上都会骗人。

### 顺带修掉：这一页从来没有自动读过

`WatchTabViewModel.Refresh()` 一直没人调——切到这一页什么都不会发生，必须手点按钮。
原来按钮叫【重算并求值】还说得通（那是个明确的"跑一遍"动作），改成纯读库之后就说不通了，
而且初始文字还写着"点【重算并求值】开始"、指着一个已经不存在的按钮。

改成：**第一次切进来自动读，之后保持上次结果**，要最新的点【刷新】。
不是每次切页都读——读一轮要扫每只票 7 张表再算一次全市场均线广度（那一项实测 630ms），
切来切去都顿一下很烦。这跟【每日晨检】当初改成全手动是同一个理由，只是这页轻得多。
