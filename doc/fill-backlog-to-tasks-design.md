...
## 8. 实机验证（2026-09-18 09:33~09:36，Debug 实例）

Debug 数据目录在 `bin/.../win-x64/data/local`，跟生产库天然隔离（起跑时四张表都是 0 行）。
全程走界面（UIAutomation 点按钮），往 Debug 的 manifest 里塞残缺日待办当输入。

### 入口一：【重新拉取失败】（走 `ITaskBacklogRunner` 回路）

| 验的是什么 | 结果 |
|---|---|
| 三项都被转交给任务，两融仍走 orchestrator | ✅ 日志收尾行：`FetchBlockTrade（任务自补）、FetchLhbSeat（任务自补）、StepLhb（任务自补）、融资余额残缺 1 天` |
| 真抓到了 | 大宗 82 行、席位 620 行、龙虎榜 62 行、两融 4105 行，各"补上 1/1 天" |
| 待办被复查后划掉 | ✅ 四条 `partial_day` 全部清空 |
| 没有误进"确认就这些"名单 | ✅ `ConfirmedPartialDays` 四项仍为空 |

**"（任务自补）"这一句就是这次改动的直接证据**——它只在 `RunFillBacklogAsync` 的转交分支里加。

### 入口二：计划项设成【只补待办】（走 `MainViewModel` 分派）

| # | 操作 | 结果 |
|---|---|---|
| 1 | 1.07【拉取龙虎榜席位】模式改【只补待办】→ 执行 | 补 2026-09-16，**730 行**，待办清空 |
| 2 | 1.28【大宗交易】同上 | 补 2026-09-16，**97 行**，待办清空 |
| 3 | 待办空了再点一次 | `⏸ 【大宗交易】本轮没开工——大宗交易没有欠着的残缺日` ← `Skipped` 语义对 |

### 顺带发现（既有问题，本次不改）

收尾那句「已排定自动重试：…（大宗残缺日 1 天 …）」用的是**清理前**的待办摘要——
补完清空了它还在报。`ScheduleAutoRetry` 取摘要的时机早于 manifest 落盘，跟本次改动无关
（两融那条走老路也一样），留着记一笔。
