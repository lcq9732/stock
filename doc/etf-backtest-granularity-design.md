...
> | | 预期 | 实测 |
> |---|---|---|
> | 匹配上库里 ETF 的基金 | 323 只 | **323 只** ✓ |
> | 会导入的事件 | — | 1121 条（现金 896 + 折算 225） |
> | `Market` 字段 vs 库里存的前缀 | 应一致 | **0 条不一致** ✓ |
> | 份额**合并**（送转比例为负） | — | **74 条 / 70 只**——正是改 `IsEmpty` 要救的那批 |
> | 首次回补请求量 | — | ≈3930 个（全抓要 13352 个，省 70%） |
