...
### ⚠ 那两个「误判残缺日」我清错了（当天稍后修正）

当时的处置：移除 `ConfirmedPartialDays["FetchMarketEvents"] = [2020-01-23, 2020-02-03]`，
理由是「复核过这两天是 58 / 56 行，与接口实测真值一致，本来就是齐的，是误判」。

**这个判断是错的，搞反了这个字段的语义。** 收尾时跑了一次日频体检，它们又被报出来了：

```
大宗交易(东财)   交易日 2602  中位行数 196  空日 0  残缺日 2
      2020-01-23  58 行（基准 306）ThinRows
      2020-02-03  56 行（基准 306）ThinRows
```

`ConfirmedPartialDays` 不是「误判名单」，而是**「数据源那天就是只有这些，以后体检不再记待办」**
（`FullAuditTask.QueuePartialDays` 用它过滤）。那两天**数据完整**和**相对基准偏少**这两件事同时成立：
58 行对基准 306，`ThinRatio` 判据会**永远**判它们残缺，而 `IsQuietDay` 豁免不住——
2020-01-23 是春节前最后一个交易日、2020-02-03 是疫情开市首日，成交额不低，但大宗本来就清淡。

删掉的后果：下次全库体检重新把它们记进待办 → 补抓 → 补回来还是 58/56 → 复查仍判残缺 →
Tries+1 → 满 2 轮后重新写回。**会自愈，代价是几轮无谓请求。**

**正确做法是改键名**（`FetchMarketEvents` → `FetchBlockTrade`），不是删。已修正：

```
ConfirmedPartialDays["FetchBlockTrade"] = [2020-01-23, 2020-02-03]
```

判断某天该不该进这个名单，看的不是「数据完不完整」，而是**「补多少轮都还是这个数」**。
...
## 7.10 中断文案按模式分岔（2026-09-18）

`OnStoppedAsync` 原来一律打「下次从水位线接着走即可」。这句话**对「整段回补」是错的**——
它不看水位线，每轮都从 `Floor` 重新排期，中断＝**进度不保留**；改用增量也接不上
（只从水位线往前回看 `LookbackDays`=30 天，够不着中间没跑到的那段）。

这个缺陷是在**席位表**那轮暴露的：2026-09-17 用户看到那句话，才放心停掉了跑到 459/2603 的
整段回补，以为下次能续上。大宗这边是同一份代码（席位表照抄的模板），所以一起改。

改法：加一个 `private FetchMode _mode` 字段在 `FetchAsync` 开头记下本轮模式，
`OnStoppedAsync` 据它分岔。测试见 `BlockTradeTaskTests` 的
`整段回补中断_要说清进度不保留` / `增量中断_说从水位线接着走`（`FakeFetcher` 加了 `CancelAt`）。
完整记录见 doc/lhb-seat-task-design.md §11。
