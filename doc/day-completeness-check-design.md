...
## 15. 最终验收：生产实例跑出来的第一份结果（2026-09-18 02:25）

前面所有验证要么是 Debug 构造数据、要么是我拿等价 SQL 预演。这是**产品代码在生产库上**
跑出来的第一份真实结果：

```
[02:25:48] 当日完整性体检：查 2026-09-17（K线三类标的 → 日更表 → 覆盖式快照）...
[02:25:48] 当日完整性体检：2026-09-17 全齐 —— 个股日K(三口径)齐；ETF日K齐；ETF日K·不复权齐；
指数日K齐；资金净流入齐；融资余额齐；龙虎榜齐；龙虎榜席位(东财)齐；大宗交易(东财)齐；
板块行情齐；总股本/流通市值齐；行业景气指标(日频)齐（45 项）（用时 29.2 秒）
```

**11 项全齐、零误报**，45 个日频指标全部判到（跟 §14.2 那次预演的样本数对得上），
manifest 里待办 0 条。

### 15.1 更正：不是"亚秒级"，是 29 秒

§3.5 写的"带日期下界之后全是亚秒级"只对**单条查询、热缓存**成立。实测把各段拆开计时
（热缓存，同一台机器）：

| 段 | 耗时 |
|---|---|
| ① K线六个面（各一次 join + NOT EXISTS） | 合计 0.9 秒（个股 day 0.42、其余各 0.1） |
| ② 五张日更表（行数 + 市场 DISTINCT） | 合计 2.1 秒（NetInflow 的行数 1.12 最重） |
| ③ 两张覆盖式快照 | 0.5 秒 |
| ④ 45 个日频指标 | 0.01 秒 |
| **合计** | **约 3.5 秒** |

真实那轮 29.2 秒，差额是**冷缓存的磁盘 IO**：体检跑在日更末尾（02:25），此前一整夜的抓取
把页缓存全冲成了写入的脏页，25GB 库上每次索引查找都要回盘。

29 秒在预算内（目录里给这一项估的是 2 分钟），是每天一次的纯本地查库，**不优化**。
留这张表是为了以后有人看见"怎么要半分钟"时知道正常值是多少、以及该去哪一段找。

### 15.2 顺带记一个不属于这条线的小毛病

那轮日志里体检前面有一句：

> 提示：【当日完整性体检】的前置【个股日K·前复权】今天还没成功跑过，这一项会用库里已有的
> 数据继续跑（结果可能偏旧…）

其实数据是齐的（锚 09-17、全齐）。这句话来自软前置的判据按**自然日**算"今天跑过没有"，
而日更是 18:00 开跑、跨夜到 02:25——那会儿"今天"已经翻篇成 09-18 了，前置当然"今天没跑过"。

跨夜的轮次里这句提示对**所有**排在后半夜的项都会出现，措辞会让人以为结果不可信。
判据本身应该跟 `DueAnchorAt` 那套"轮次锚点"对齐（那套就是为解决同一个问题做的）。
这是 `PlanRunner` 软前置提示的事，不在当日完整性体检这条线里，另外记着。

---

## 16. 修掉那条假提示：前置判据也按轮次算（2026-09-18）

§15.2 记的那个小毛病。查下来它不是措辞问题，而是**同一个根因在前置判据上重演**——
`AlreadyRanOn` 2026-09-01 已经从"落在哪一天"改成了轮次锚点，前置那两处漏改了。

### 16.1 两个方向都出过事

| 判据 | 原来 | 跨夜时的后果 |
|---|---|---|
| 软前置（`WarnIfSoftDependencyStale`） | `(LastStart ?? LastEnd)?.Date == DateTime.Today` | **假警告**：日更 18:00 开跑、前置 19:00 跑完，排到凌晨 02:25 的项被判成"前置今天没跑过"，挂一句"结果可能偏旧"——而那轮体检 11 项全齐（2026-09-18 真实日志） |
| 硬前置（`DependencyFailedToday`） | `LastEnd?.Date == DateTime.Today` | **漏拦**：前置昨晚 18:30 失败，凌晨排到的项判成"今天没失败过"照跑不误，可它要的数据压根没抓到 |

后一个比前一个严重：假警告只是吵，漏拦是真的让下游拿着残缺数据跑完还报绿。

### 16.2 判据下沉到模型层

两条都改成跟**当期锚点**比，而且放进 `FetchPlanItem`，跟 `AlreadyRanOn` 并排：

```csharp
public DateTime RoundAnchor(DateTime now)          // 这一轮什么时候开的工
public bool RanOkSince(DateTime anchor)            // 软前置：本轮跑成功过没有（比开始时刻）
public bool FailedSince(DateTime anchor)           // 硬前置：本轮跑过且失败了没有（比结束时刻）
```

放模型层不只是为了整齐——**这样才测得了跨夜**。判据留在 `PlanRunner` 里就只能靠跑整个引擎，
而引擎读的是 `DateTime.Now`，没法把时间拨到凌晨两点。

两个取舍写在代码里，这里记要点：

- **`RoundAnchor` 拿不到锚点时退回"今天零点"**，不能直接用 `DueAnchorAt` 的返回值：
  手动项和「仅一次」拿到的是 `DateTime.MinValue`，拿它当锚的话"任何时候跑过"都算本轮跑过——
  比按自然日判还松。
- **软前置比开始时刻、硬前置比结束时刻**：前者跟 `AlreadyRanOn` 同理（跨夜长任务昨晚开工今天收工，
  拿结束时刻比会算成新一轮跑的）；后者是因为失败在跑完那一刻才定，而且跳过那条路径会把
  `LastStart` 清空（PlanRunner 里那段防空转的注释）。

### 16.3 一个容易掉进去的坑：测试得把前置和被挡项放**同一组**

`FetchPlanGroup.DueAnchorAt` 往回看昨晚那一轮时，要求**这一组已经有项开工过**
（`StartedSince`）才认那个锚——没开过工就谈不上"当前在那一轮里"，这道闸门是为了不让
「不早于 18:00」被架空。

所以拿两个各自独立的组去测，锚点会回 null、退化成自然日，测试看着像判据没生效。
真实形状本来就是同组（日更组里【个股日K】排在【当日完整性体检】前面），测试照着造就对了。

### 16.4 验证

- 单测 4 条（跨夜前置成功不该报 / 前置停在上一轮要报 / 跨夜硬前置失败仍要拦 /
  手动项锚点退回当天零点）；全量 **1,689 个全过**。
- 实机（Debug 实例）造跨夜场景——日更组 NotBefore 18:00、当前是上午 08:27，
  锚点回看到昨天 18:00：

  | 场景 | 日志 |
  |---|---|
  | 前置**昨晚这一轮**跑成功 | 没有提示（修复前必然误报一句） |
  | 前置停在**上一轮** | `提示：【当日完整性体检】的前置【个股日K·前复权】这一轮还没成功跑过…` |

  顺带看到锚点本身也对了：`上一轮（09-17 18:00 起）有 1 项没跑成，今天这一轮会一起重跑`。
  验证用的 `fetch-plan.json` 已还原。
