...
**⚠ 上线当天踩到的第二个坑：`CheckDays` 不能截日历**（2026-09-16 修）

第一版 `CheckDays` 把日历截断到"要查的最后一天"，理由写的是"复查跟最近有没有落后无关"。
这句话只对了一半——尾部滞后确实无关，但 cutoff 还决定**日历长度**，而两条判据都依赖前后窗口：

- `TrailingMedian`：前面凑不够 `ThinWindowRadius` 个非空日时会**往后**补
- `IsQuietDay`：取前后各 `QuietWindowRadius` 天的成交额中位数

日历一截，后面的样本全没了，复查就比体检宽松。真实后果：`Lhb` 2005-11-14 被复查判"已补齐"
划掉，下一轮体检又报出来，`Tries` 每轮重置成 0——那天在「补 → 复查说齐 → 划掉 → 体检又报」
之间**永远循环、永不收敛**，补不回来的日子会一直占着待办。

这个方法存在的全部意义就是"跟体检同一套判据"，截日历等于把这个前提拆掉了。
所以日历必须读完整（cutoff 传 `DateTime.Today`），只在判定时挑指定的那几天。

**守它的测试**：`复查判据与体检判据_逐天结论一致` —— 逐天问一遍 `CheckDays`，
结论集合必须跟 `Check` 完全相同。不比具体数值，只比"报不报"；比数值会把测试绑死在实现上，
而这里要守的恰恰是"两条路径等价"这件事本身。
