...
| **StockPlatform.Analyzer**（`src/StockPlatform.Analyzer`） | 批量分析程序：读取本地数据库，按用户选定的粒度批量分析全市场股票，通过 Tab 切换"筑基法"/"金叉法"两种分析方法，图表化展示判断依据 | [doc/analysis-app-design.md](doc/analysis-app-design.md) |
...
两个可执行程序分别是 `StockPlatform.Fetcher.exe`、`StockPlatform.Analyzer.exe`（发布命令见各自文档）。

## 文档索引

- [doc/data-platform-design.md](doc/data-platform-design.md) — 数据获取程序的整体设计：多数据源、master/daily文件方案、网盘同步调研、限速与反封IP策略
- [doc/analysis-app-design.md](doc/analysis-app-design.md) — 批量分析程序的设计：筑基法/金叉法两种分析规则、图表化依据展示
